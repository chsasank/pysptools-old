...
def mATGP(data, q, mask):
    ...
    Parameters:
        ...
        mask: `numpy array`
            A binary mask, if True the corresponding spectrum is part of the
            endmembers search.
...
def mNFINDR(data, mask, q, transform=None, maxit=None, ATGP_init=False):
    """
    ...
    Returns: `tuple: numpy array, numpy array, numpy array, int`
        * Set of induced endmembers (N x p)
    ...
    if transform == None:
        ...
        transform = data
        transform = _PCA_transform(data, q-1)
    else:
        transform = transform
    ...
    IDX = None
    if ATGP_init == True:
        induced_em, idx = mATGP(transform, q, mask)
